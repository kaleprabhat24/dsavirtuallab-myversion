...
const questions = [
  {
    ...
    description: "Write a function to perform BFS traversal of a binary tree. The BFS traversal visits nodes level by level from left to right.",
    difficulty: "Medium",
    examples: [
...
void bfsTraversal(struct Node* root) {
    ...
    struct QueueNode* front = newQueueNode(root);
    ...
    while (front != NULL) {
        ...
        if (current->left != NULL) {
            rear->next = newQueueNode(current->left);
            rear = rear->next;
        }
        if (current->right != NULL) {
            rear->next = newQueueNode(current->right);
            rear = rear->next;
        }
...
export default function TreePracticePlatform() {
